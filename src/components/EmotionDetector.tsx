...
const EmotionDetector: React.FC = () => {
  ...
  const [originalResult, setOriginalResult] = useState<EmotionResult | null>(null); // ✅ Add
  const [translatedResult, setTranslatedResult] = useState<EmotionResult | null>(null); // ✅ Add
  ...
  useEffect(() => {
    const translateUIAndResult = async () => {
      // Translate UI text
      if (language !== 'en') {
      ...
      // Translate emotion result (if available)
      if (originalResult) {
        if (language === 'en') {
          setTranslatedResult(originalResult);
        } else {
          const [translatedPrimary, translatedInsights, translatedTips] = await Promise.all([
            translateText(originalResult.primaryEmotion, language),
            translateText(originalResult.insights, language),
            Promise.all(originalResult.copingTips.map(tip => translateText(tip, language))),
          ]);

          setTranslatedResult({
            ...originalResult,
            primaryEmotion: translatedPrimary,
            insights: translatedInsights,
            copingTips: translatedTips,
          });
        }
      }
    };

    translateUIAndResult();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setOriginalResult(analysis); // 👈 Save only original
      setShowResult(true);
    } catch (error) {
      console.error(error);
    } finally {
